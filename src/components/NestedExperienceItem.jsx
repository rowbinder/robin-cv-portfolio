...
const NestedExperienceCard = ({ company, logoPath, roles = [] }) => {
  return (
    <div className="p-4 border-b border-gray-200 last:border-0 hover:bg-gray-50/30 transition-colors text-left font-[-apple-system,system-ui,BlinkMacSystemFont,'Segoe_UI',Roboto,'Helvetica_Neue','Fira_Sans',Ubuntu,Oxygen,'Oxygen_Sans',Cantarell,sans-serif]">

      {/* Company Header Row: Logo + Name */}
      <div className="flex gap-4 mb-4">
        ...
        <h2 className="text-[16px] font-bold text-gray-900 mt-1">{company}</h2>
      </div>

      {/* Timeline + Roles Row */}
      <div className="flex gap-4">

        {/* Left column: just the vertical line, no dots */}
        <div className="w-12 flex-shrink-0 flex justify-center">
          <div className="w-[2px] bg-gray-200 h-full" />
        </div>

        {/* Right column: roles, each with its own dot that bleeds left onto the line */}
        <div className="flex-1 min-w-0 space-y-8 pb-2">
...
const NestedRoleItem = ({ role, duration, location = "Melbourne, Australia", description, bullets = [], skills = [] }) => {
  ...
  return (
    // Relative container, dot bleeds left by half the gap (gap-4 = 16px) + half dot width
    <div className="relative">
      {/* Dot: sits on the line in the left column */}
      <div className="absolute -left-[21px] top-[5px] w-[9px] h-[9px] bg-gray-300 rounded-full z-10 border-2 border-white" />
